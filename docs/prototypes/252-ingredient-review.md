# #252 — does checking ingredients before transfer save work?

Throwaway branch `prototype/252-ingredient-review`, based on merged #260. The
only optional-review alternative is D. No production feature or purchase
contract is accepted. #242 remains a decision gate; #254 remains deferred.
...
Open http://localhost:9252/recipes/menus/prototype-review?variant=B and sign in
with **review252** / **local-review-252**. The prepared worktree has
dependencies linked and a safe `.env.example` copy. In a fresh checkout, first
run `bun install` and `cp .env.example .env`. The one-command runner generates
the client, migrates a dedicated scratch database and seeds synthetic data once.

All comparison actions are in browser memory. Changing variant, scope or kitchen
facts resets that trial. The prototype route returns 404 in production builds.
The real-app baseline uses only `/private/tmp/qm-252-review/data.db` and the
same synthetic Recipes/Menu. No shared household or production records are
touched.
